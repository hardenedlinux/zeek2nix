...
let
  ...
  pname = "zeek";
  plugin = import ./plugin.nix { inherit plugins linuxHeaders llvmPackages confDir zeekctl checkPlugin args; };
in
stdenv.mkDerivation
  rec {
    ...
    buildInputs =
      [ openssl libpcap zlib curl libmaxminddb gperftools python38 swig caf ncurses5 ]
        ...
        ''
          for e in $(cd $out/bin && ls |  grep -E 'spicy|hilti' ); do
            wrapProgram $out/bin/$e \
              ...
              --set LIBRARY_PATH    "${
          lib.makeLibraryPath
            [ flex bison python38 zlib glibc llvmPackages.libclang llvmPackages.libcxxabi llvmPackages.libcxx ]
        }"
           done
        ''
      else "";
