{
  ...
  nixConfig = { flake-registry = "https://github.com/hardenedlinux/flake-registry/raw/main/flake-registry.json"; };
  inputs = {
    ...
    @ inputs:
    {
      overlay =
        ...
        rec {
          ...
          zeek-latest =
            final.zeek-release.overrideAttrs (old: rec { inherit (final.zeek-sources.zeek-latest) src pname version; });
            ...
            dockerTools.buildImage
              {
                name = "zeek-docker";
                tag = "latest";
                contents = [ zeek-release coreutils zsh ];
                runAsRoot = ''
                  #!${pkgs.runtimeShell}
                  ${./pre-run-zeekctl.sh}
                '';
        ...
        (
          ...
          in
            rec {
              inherit pkgs;
              packages =
                inputs.flake-utils.lib.flattenTree rec { inherit (pkgs) zeek-release zeek-latest; }
            ...
            imports = [
              {
                nixpkgs.config.packageOverrides =
                  pkgs: { inherit (inputs.self.packages."${pkgs.system}") zeek-release; };
              }
              ./module
            ];
          };
